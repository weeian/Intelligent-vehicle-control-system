...
u8 	RmtSta=0;	  	  
u16 Dval;		//下降沿时计数器的值
u32 RmtRec=0;	//红外接收到的数据，用来保存用户码、用户码、数据码、数据反码	    
u8  RmtCnt=0;	//按键按下的次数	  
//定时器2中断服务程序	 
void TIM4_IRQHandler(void)
{ 		    	 
    if(TIM_GetITStatus(TIM4,TIM_IT_Update)!=RESET)//定时计数器翻转
	{
...
//	 0,没有任何按键按下
//其他,按下的按键键值.
u8 Remote_Scan(void)//按键扫描程序
{        
	u8 sta=0;       
    u8 t1,t2;  
	if(RmtSta&(1<<6))//得到一个按键的所有信息了，看RmtSta的第六位是否为1，若为1，则认为得到了键值
	{ 
	    t1=RmtRec>>24;			//得到地址码
